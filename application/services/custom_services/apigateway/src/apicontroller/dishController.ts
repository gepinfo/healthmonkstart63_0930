...
export class dishController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/dish/:id', this.GpDelete);
this.router.get('/dish/get/search', this.GpSearch);
this.router.put('/dish/get/update', this.GpSearchForUpdate);
this.router.put('/dish', this.GpUpdate);
this.router.get('/dish', this.GpGetAllValues);
this.router.post('/dish', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into dishController.ts: GpDelete');
        new ApiAdapter().delete(Constant.DISHMANAGERURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.DISHMANAGERURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdapter().put(Constant.DISHMANAGERURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.DISHMANAGERURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
